...
public class SelectDeviceActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private ArrayList<String> deviceMacs,deviceMacs1 ; // 数据源 ： 所有扫描到的设备mac地址
    ...
    private void initBle() {
        ...
        //scanManager.addScanFilterCompats(new ScanFilterCompat.Builder().setDeviceName("BLE#0x0C61CF397E0E").build());
        ...
        scanManager.setScanCallbackCompat(new ScanCallbackCompat() {
            ...
            public void onScanResult(int callbackType, ScanResultCompat result) {
                ...
                if (!deviceMacs.contains(result.getDevice().getAddress())) {
                    //deviceMacs.add(result.getDevice().toString());
                    deviceMacs.add(result.getDevice().getAddress());
                    deviceMacs1.add(result.getDevice().getName()+"-"+result.getDevice().getAddress());
    ...
    private void initView() {
        ...
        adapter = new ScanDeviceAdapter(deviceMacs1);
        ...
        adapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                if (!selectDeviceMacs.contains(deviceMacs.get(position))){
                    ...
                    ((TextView)view.findViewById(R.id.txtState)).setTextColor(getResources().getColor(R.color.colorAccent));
                    //Toast.makeText(SelectDeviceActivity.this, "已选择", Toast.LENGTH_SHORT).show();
                }else {
                    selectDeviceMacs.remove(deviceMacs.get(position));
                    ((TextView)view.findViewById(R.id.txtState)).setText("未选择");
                    ((TextView)view.findViewById(R.id.txtState)).setTextColor(Color.parseColor("#FFFFFF"));
                    //Toast.makeText(SelectDeviceActivity.this, "未选择", Toast.LENGTH_SHORT).show();
                }
            }
        });
...
//                scanManager.startCycleScan(); //不会立即开始，可能会延时
            ...
            case R.id.btnOk:
                ...
                //HashSet<String> hs = new HashSet<String>(selectDeviceMacs);
