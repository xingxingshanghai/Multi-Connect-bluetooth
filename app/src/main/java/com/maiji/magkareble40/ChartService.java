...
public class ChartService {
    ...
    private int i=0;
    private int pos = 0;
    public ChartService(Context context,int temp) {
    ...
    public void setXYMultipleSeriesRenderer(double maxX, double maxY, String chartTitle, String xTitle, String yTitle, int axeColor, int labelColor, int curveColor, int gridColor) {
       ...
       // multipleSeriesRenderer.setMargins(new int[] { 20（上）, 30(左), 15(下), 20(右) });
        ...
        multipleSeriesRenderer.setMargins(new int[] { 28, 35, 5, 5 });
            ...
            multipleSeriesRenderer.setMargins(new int[] { 2, 35, 5, 5 });
...
//        multipleSeriesRenderer.setPanLimits(new double[] { 0, 4500, -2, 4 });//设置拉动的范围
    ...
    protected void updateCharts(double datas[], int MAX_POINT) {
        ...
        if (i < MAX_POINT) {
            ...
            multipleSeriesRenderer.setXAxisMin(mSeries.getItemCount() - MAX_POINT);
            multipleSeriesRenderer.setXAxisMax(mSeries.getItemCount());
        }
